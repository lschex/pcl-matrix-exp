...
print "<p>You will see a series of scenes. Each scene contains five image pairs. There will be a black bar connecting each pair - this represents two objects that are \"with\" each other. Above these pairs is an audio control button. Press play on the audio control button to receive instructions about which image to pick. You will be able to choose a picture once the instructions have finished playing.</p>";

print "<p>In some cases, part of the instructions will be covered up by quiet static noise, so you won't be able to hear one of the words. This is not an error; it is intended. In that case, just do your best and trust your intuition.</p>";
